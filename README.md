# [uzhotels](http://joshdmiller.github.com/ng-boilerplate) [![Build Status](https://api.travis-ci.org/ngbp/ngbp.png?branch=v0.3.2-release)](https://travis-ci.org/ngbp/ngbp)
...
`uzhotels` is designed to make life easy by providing a basic framework
...
uzhotels also comes prepackaged with the most popular design frameworks
...
The principal goal of `uzhotels` is to set projects up for long-term
success.  So `uzhotels` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `uzhotels`.

But `uzhotels` is not an example of an AngularJS app: this is a
...
getting `uzhotels` up and running. Though `uzhotels` is really simple
...
`uzhotels` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `uzhotels`, like Twitter Bootstrap
...
Technically, `uzhotels` is now ready to go.
...
`uzhotels`. What follows in this section is a quick introduction to the
...
`uzhotels` also includes [Live Reload](http://livereload.com/), so you no
...
default with `uzhotels`) you need to tell Live Reload to allow it. Go to
