# The Tools Used in `uzhotels`

## Introduction

`uzhotels` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `uzhotels` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`uzhotels` uses git to manage its codebase. While in theory you don't have
to use Git once you download `uzhotels`, this project makes the assumption
...
While `uzhotels` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `uzhotels` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `uzhotels`; Grunt is
...
to get started with `uzhotels`. But for those curious or looking to go a
...
Grunt is the engine behind `uzhotels`. It's the magic that makes it move.
...
`uzhotels` comes with a `bower.json` file that looks something like this:
...
in `uzhotels` by default in there to see what I mean.
